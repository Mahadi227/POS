/**
 * Serial numbers — trace serialized batch units
 */
document.addEventListener('DOMContentLoaded', () => {
    const tableWrap = document.getElementById('whSerTableWrap');
    if (!tableWrap) return;
    ...
    const els = {
        search: document.getElementById('whSerSearch'),
        warehouse: document.getElementById('whSerWarehouse'),
        status: document.getElementById('whSerStatus'),
        refresh: document.getElementById('whSerRefreshBtn'),
        exportBtn: document.getElementById('whSerExportBtn'),
        newBtn: document.getElementById('whSerNewBtn'),
        heroMeta: document.getElementById('whSerHeroMeta'),
        breakdownPanel: document.getElementById('whSerBreakdownPanel'),
        statusChips: document.getElementById('whSerStatusChips'),
        statTotal: document.getElementById('whSerStatTotal'),
        statActive: document.getElementById('whSerStatActive'),
        statInactive: document.getElementById('whSerStatInactive'),
        statProducts: document.getElementById('whSerStatProducts'),
        loading: document.getElementById('whSerLoading'),
        empty: document.getElementById('whSerEmpty'),
        pagination: document.getElementById('whSerPagination'),
        prev: document.getElementById('whSerPrev'),
        next: document.getElementById('whSerNext'),
        pageMeta: document.getElementById('whSerPageMeta'),
        createModal: document.getElementById('whSerCreateModal'),
        createClose: document.getElementById('whSerCreateClose'),
        createCancel: document.getElementById('whSerCreateCancel'),
        createForm: document.getElementById('whSerCreateForm'),
        formWarehouse: document.getElementById('whSerFormWarehouse'),
        formProduct: document.getElementById('whSerFormProduct'),
        formSerial: document.getElementById('whSerFormSerial'),
        detailModal: document.getElementById('whSerDetailModal'),
        detailClose: document.getElementById('whSerDetailClose'),
        detailTitle: document.getElementById('whSerDetailTitle'),
        detailSubtitle: document.getElementById('whSerDetailSubtitle'),
        detailBody: document.getElementById('whSerDetailBody'),
        detailActions: document.getElementById('whSerDetailActions'),
        recallBtn: document.getElementById('whSerRecallBtn'),
        depleteBtn: document.getElementById('whSerDepleteBtn'),
        expiredBtn: document.getElementById('whSerExpiredBtn'),
        toast: document.getElementById('whSerToast'),
    };

    function toast(msg, type = 'success') {
        if (!els.toast) return;
        els.toast.textContent = msg;
        els.toast.className = `wh-ser-toast show${type === 'error' ? ' wh-ser-toast--error' : ''}`;
    ...
    function expiryCell(row) {
        ...
        let cls = '';
        if (days != null) {
            if (days < 0) cls = 'wh-ser-expiry--past';
            else if (days <= 30) cls = 'wh-ser-expiry--soon';
        }
        const hint = days != null ? ` <small class="wh-ser-expiry-days">(${days}d)</small>` : '';
        return `<span class="wh-ser-expiry ${cls}">${esc(formatDate(exp))}${hint}</span>`;
    }

    function setStatsLoading(loading) {
        document.querySelectorAll('.wh-ser-stat__value').forEach((el) => el.classList.toggle('is-loading', loading));
    }
    ...
    function renderStats(summary) {
        ...
        if (els.statProducts) els.statProducts.textContent = String(s.products ?? 0);
        if (els.heroMeta) {
            els.heroMeta.textContent = t('wh_ser_hero_meta', s.active ?? 0, s.products ?? 0);
        }
    ...
    function renderBreakdown(items) {
        ...
        els.statusChips.innerHTML = sorted.map((r) => {
            ...
            return `<button type="button" class="wh-ser-status-chip${isActive ? ' is-active' : ''}" data-status="${esc(r.status)}">
                <span>${esc(statusLabel(r.status))}</span>
                <strong>${Number(r.count || 0)}</strong>
            </button>`;
        }).join('');
        els.statusChips.querySelectorAll('.wh-ser-status-chip').forEach((btn) => {
    ...
    function renderTable(items) {
        ...
        tableWrap.innerHTML = `<table class="modern-table wh-table wh-ser-table">
<thead><tr>
    <th>${esc(t('wms_col_serial'))}</th>
...
<tbody>${items.map((r) => `<tr>
    <td><strong class="wh-ser-serial">${esc(r.serial_number)}</strong></td>
    ...
    <td class="wh-ser-row-actions">
        <button type="button" class="wh-btn wh-btn--ghost wh-btn--sm" data-ser-view="${r.id}">${esc(t('wms_view_details'))}</button>
    </td>
</tr>`).join('')}</tbody></table>`;

        tableWrap.querySelectorAll('[data-ser-view]').forEach((btn) => {
            btn.addEventListener('click', () => openDetail(Number(btn.dataset.serView)));
        });
    }
    ...
    async function loadProducts() {
        ...
        if (!els.formProduct) return;
        els.formProduct.innerHTML = `<option value="">${esc(t('wms_select_product'))}</option>`
            + state.products.map((p) => `<option value="${p.id}">${esc(p.name)}${p.sku ? ` (${p.sku})` : ''}</option>`).join('');
    }
    ...
    function buildExportRows(items) {
        return [
            [t('wms_col_serial'), t('wms_col_product'), t('wms_col_batch'), t('wms_nav_warehouses'), t('wms_col_qty'), t('wms_col_expiry'), t('col_status')],
            ...items.map((r) => [r.serial_number, r.product_name, r.batch_number, r.warehouse_name, r.quantity, r.expiry_date, r.status]),
        ];
    }
    ...
    async function exportData() {
        try {
            ...
            exportCsv(`serial-numbers-${new Date().toISOString().slice(0, 10)}.csv`, buildExportRows(items));
        } catch (e) {
            showError(e.message || t('load_error'));
        }
    }
    ...
    async function openDetail(id) {
        ...
        try {
            ...
            const r = res.data;
            if (els.detailTitle) els.detailTitle.textContent = r.serial_number || t('wms_batch_details');
            if (els.detailSubtitle) {
                els.detailSubtitle.textContent = [r.batch_number, r.product_name, r.warehouse_name].filter(Boolean).join(' · ');
            }
            const days = r.days_to_expiry != null ? Number(r.days_to_expiry) : null;
            els.detailBody.innerHTML = `
                <dl class="wh-ser-detail-grid">
                    <div><dt>${esc(t('wms_col_serial'))}</dt><dd><strong>${esc(r.serial_number || '—')}</strong></dd></div>
            ...
            els.detailBody.innerHTML = `<p class="wh-ser-empty-inline">${esc(e.message || t('load_error'))}</p>`;
        }
    }
    ...
    async function openCreate() {
        await loadProducts();
        await loadFormWarehouses();
        els.createForm?.reset();
        if (els.formSerial) els.formSerial.focus();
    ...
    els.createForm?.addEventListener('submit', async (e) => {
        ...
        if (!String(data.serial_number || '').trim()) {
            toast(t('wh_ser_serial_required'), 'error');
            els.formSerial?.focus();
            return;
        }
