/**
 * Warehouse transfers hub — create and manage all stock transfers
 */
document.addEventListener('DOMContentLoaded', () => {
    const tableWrap = document.getElementById('whTrfTableWrap');
    if (!tableWrap) return;
    ...
    const els = {
        search: document.getElementById('whTrfSearch'),
        warehouse: document.getElementById('whTrfWarehouse'),
        status: document.getElementById('whTrfStatus'),
        refresh: document.getElementById('whTrfRefreshBtn'),
        exportBtn: document.getElementById('whTrfExportBtn'),
        newBtn: document.getElementById('whTrfNewBtn'),
        heroMeta: document.getElementById('whTrfHeroMeta'),
        breakdownPanel: document.getElementById('whTrfBreakdownPanel'),
        statusChips: document.getElementById('whTrfStatusChips'),
        statTotal: document.getElementById('whTrfStatTotal'),
        statRequested: document.getElementById('whTrfStatRequested'),
        statProgress: document.getElementById('whTrfStatProgress'),
        statCompleted: document.getElementById('whTrfStatCompleted'),
        loading: document.getElementById('whTrfLoading'),
        empty: document.getElementById('whTrfEmpty'),
        pagination: document.getElementById('whTrfPagination'),
        prev: document.getElementById('whTrfPrev'),
        next: document.getElementById('whTrfNext'),
        pageMeta: document.getElementById('whTrfPageMeta'),
        createModal: document.getElementById('whTrfCreateModal'),
        createClose: document.getElementById('whTrfCreateClose'),
        createCancel: document.getElementById('whTrfCreateCancel'),
        createForm: document.getElementById('whTrfCreateForm'),
        type: document.getElementById('whTrfType'),
        fromWh: document.getElementById('whTrfFromWh'),
        toWh: document.getElementById('whTrfToWh'),
        fromStore: document.getElementById('whTrfFromStore'),
        toStore: document.getElementById('whTrfToStore'),
        addLine: document.getElementById('whTrfAddLine'),
        productFilter: document.getElementById('whTrfProductFilter'),
        lineItems: document.getElementById('whTrfLineItems'),
        linesEmpty: document.getElementById('whTrfLinesEmpty'),
        lineCount: document.getElementById('whTrfLineCount'),
        estTotal: document.getElementById('whTrfEstTotal'),
        formError: document.getElementById('whTrfFormError'),
        detailModal: document.getElementById('whTrfDetailModal'),
        detailClose: document.getElementById('whTrfDetailClose'),
        detailTitle: document.getElementById('whTrfDetailTitle'),
        detailSubtitle: document.getElementById('whTrfDetailSubtitle'),
        detailBody: document.getElementById('whTrfDetailBody'),
        toast: document.getElementById('whTrfToast'),
    };

    function toast(msg, type = 'success') {
        if (!els.toast) return;
        els.toast.textContent = msg;
        els.toast.className = `wh-trf-toast show${type === 'error' ? ' wh-trf-toast--error' : ''}`;
    ...
    function setStatsLoading(loading) {
        document.querySelectorAll('.wh-trf-stat__value').forEach((el) => el.classList.toggle('is-loading', loading));
    }
    ...
    function renderStats(summary) {
        ...
        if (els.heroMeta) {
            els.heroMeta.textContent = t('wh_trf_hero_meta', s.requested ?? 0, s.in_progress ?? 0);
        }
        setStatsLoading(false);
    }

    function chipStatusFor(status) {
        if (status === 'requested') return 'trf_pending';
        return status;
    }
    ...
    function renderBreakdown(items) {
        ...
        els.statusChips.innerHTML = sorted.map((r) => {
            ...
            return `<button type="button" class="wh-trf-status-chip${isActive ? ' is-active' : ''}" data-status="${esc(chipStatus)}">
                <span>${esc(statusLabel(r.status))}</span>
                <strong>${Number(r.count || 0)}</strong>
            </button>`;
        }).join('');
        els.statusChips.querySelectorAll('.wh-trf-status-chip').forEach((btn) => {
    ...
    function renderTable(items) {
        ...
        tableWrap.innerHTML = `<table class="modern-table wh-table wh-trf-table">
<thead><tr>
...
<tbody>${items.map((r) => {
    ...
    return `<tr>
        ...
        <td class="wh-trf-row-actions">
            <button type="button" class="wh-btn wh-btn--ghost wh-btn--sm" data-trf-view="${r.id}">${esc(t('wms_view_details'))}</button>
            ${canApproveRow ? `<button type="button" class="wh-btn wh-btn--sm" data-trf-approve="${r.id}">${esc(t('wms_approve'))}</button>` : ''}
            ${canCompleteRow ? `<button type="button" class="wh-btn wh-btn--sm wh-btn--primary" data-trf-complete="${r.id}">${esc(t('wms_complete'))}</button>` : ''}
            ${canRejectRow ? `<button type="button" class="wh-btn wh-btn--sm wh-btn--warn" data-trf-reject="${r.id}">${esc(t('wms_reject'))}</button>` : ''}
        </td>
    </tr>`;
}).join('')}</tbody></table>`;

        tableWrap.querySelectorAll('[data-trf-view]').forEach((btn) => {
            btn.addEventListener('click', () => openDetail(Number(btn.dataset.trfView)));
        });
        tableWrap.querySelectorAll('[data-trf-approve]').forEach((btn) => {
            btn.addEventListener('click', () => approveTransfer(Number(btn.dataset.trfApprove)));
        });
        tableWrap.querySelectorAll('[data-trf-complete]').forEach((btn) => {
            btn.addEventListener('click', () => completeTransfer(Number(btn.dataset.trfComplete)));
        });
        tableWrap.querySelectorAll('[data-trf-reject]').forEach((btn) => {
    ...
    async function exportData() {
        try {
            ...
            exportCsv(`warehouse-transfers-${new Date().toISOString().slice(0, 10)}.csv`, buildExportRows(items));
        } catch (e) {
            showError(e.message || t('load_error'));
        }
    }

    function resetTrfSwipeStyles() {
    ...
    async function completeTransfer(id, fromDetail = false) {
        ...
        hideError();
        toast(t('wh_trf_toast_completed'));
        if (fromDetail) closeModal(els.detailModal);
        await load();
    }
    ...
    async function openDetail(id) {
        ...
        try {
            ...
            els.detailBody.innerHTML = `
                ...
                ${r.reason ? `<p class="wh-trf-detail-notes"><strong>${esc(t('wms_col_reason'))}:</strong> ${esc(r.reason)}</p>` : ''}
                <div class="wh-trf-lines-wrap">
                    <table class="modern-table wh-table wh-trf-lines-table">
                        <thead><tr>
                        ...
                        <tbody>${items.map((i) => `<tr>
                            ...
                            <td>${i.quantity_sent || 0}</td>
                            <td>${i.quantity_received || 0}</td>
                        </tr>`).join('')}</tbody>
                    </table>
                </div>
                ${canApproveRow || canCompleteRow || canRejectRow ? `<div class="wh-trf-detail-actions">
                    ${canApproveRow ? `<button type="button" class="wh-btn" id="whTrfApproveBtn">${esc(t('wms_approve'))}</button>` : ''}
                    ${canCompleteRow ? `<button type="button" class="wh-btn wh-btn--primary" id="whTrfCompleteBtn">${esc(t('wms_complete'))}</button>` : ''}
                    ${canRejectRow ? `<button type="button" class="wh-btn wh-btn--warn" id="whTrfRejectBtn">${esc(t('wms_reject'))}</button>` : ''}
                </div>` : ''}`;
            document.getElementById('whTrfApproveBtn')?.addEventListener('click', () => approveTransfer(r.id, true));
            document.getElementById('whTrfCompleteBtn')?.addEventListener('click', () => completeTransfer(r.id, true));
            document.getElementById('whTrfRejectBtn')?.addEventListener('click', () => rejectTransfer(r.id, true));
        } catch (e) {
            els.detailBody.innerHTML = `<p class="wh-trf-empty-inline">${esc(e.message || t('load_error'))}</p>`;
        }
    }
    ...
    async function loadFormWarehouses() {
        ...
        const fill = (sel) => {
            ...
            else if (window.WH_PAGE?.warehouseId) sel.value = String(window.WH_PAGE.warehouseId);
        };
        fill(els.fromWh);
        fill(els.toWh);
    }
    ...
    async function loadStores() {
        ...
        const fill = (sel) => {
            ...
            if (cur) sel.value = cur;
            else if (window.WH_PAGE?.storeId) sel.value = String(window.WH_PAGE.storeId);
        };
        fill(els.fromStore);
        fill(els.toStore);
    }
    ...
    function toggleTypeFields() {
        const type = els.type?.value || 'warehouse_to_warehouse';
        const setField = (fieldId, selectEl, visible, required) => {
            const field = document.getElementById(fieldId);
            if (field) field.hidden = !visible;
            if (selectEl) {
                selectEl.required = !!required && visible;
                if (!visible) selectEl.value = '';
            }
        };
        setField('whTrfFromWhField', els.fromWh, ['warehouse_to_warehouse', 'warehouse_to_store'].includes(type), true);
        setField('whTrfToWhField', els.toWh, ['warehouse_to_warehouse', 'store_to_warehouse'].includes(type), true);
        setField('whTrfFromStoreField', els.fromStore, ['store_to_warehouse', 'branch_to_branch'].includes(type), true);
        setField('whTrfToStoreField', els.toStore, ['warehouse_to_store', 'branch_to_branch'].includes(type), true);
    }
    ...
    function syncTrfLayout(rowsCount) {
        const modal = els.createModal?.querySelector('.wh-grn-modal');
        const titleEl = document.getElementById('whTrfLinesTitle');
    ...
    function addLineRow(data = {}) {
        ...
        const scrollEl = document.getElementById('whTrfLinesScroll');
        requestAnimationFrame(() => {
    ...
    function bindTrfSwipeDismiss() {
        const overlay = els.createModal;
        const panel = overlay?.querySelector('.wh-grn-modal');
        if (!overlay || !panel || panel.dataset.trfSwipeBound) return;
        panel.dataset.trfSwipeBound = '1';
        ...
        function snapBack() {
            ...
            setTimeout(resetTrfSwipeStyles, 240);
        }
    ...
    async function openCreateModal() {
        ...
        state.lineIndex = 0;
        clearFormError();
        document.getElementById('whTrfMetaWrap')?.removeAttribute('open');
        toggleTypeFields();
        addLineRow();
        updateLineSummary();
    ...
    if (canCreate) {
        ...
        els.type?.addEventListener('change', toggleTypeFields);
