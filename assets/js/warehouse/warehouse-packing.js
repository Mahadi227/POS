/**
 * Warehouse packing — verify picked lines and mark dispatches as packed
 */
document.addEventListener('DOMContentLoaded', () => {
    const tableWrap = document.getElementById('whPackTableWrap');
    if (!tableWrap) return;

    const { t, esc, money, showError, hideError, setMigrationHint, updateLastUpdated, exportCsv, loadWarehouseOptions } = WarehouseUI;
    const canPack = !!window.WH_PAGE?.canDispatch && !window.WH_PAGE?.readOnly;

    const STATUS_KEYS = {
        picking: 'wms_status_picking',
        packed: 'wms_status_packed',
    };
    const QUEUE_ORDER = ['picking', 'packed'];
    ...
    const els = {
        search: document.getElementById('whPackSearch'),
        warehouse: document.getElementById('whPackWarehouse'),
        status: document.getElementById('whPackStatus'),
        refresh: document.getElementById('whPackRefreshBtn'),
        exportBtn: document.getElementById('whPackExportBtn'),
        heroMeta: document.getElementById('whPackHeroMeta'),
        breakdownPanel: document.getElementById('whPackBreakdownPanel'),
        statusChips: document.getElementById('whPackStatusChips'),
        statQueue: document.getElementById('whPackStatQueue'),
        statPacked: document.getElementById('whPackStatPacked'),
        statTotal: document.getElementById('whPackStatTotal'),
        loading: document.getElementById('whPackLoading'),
        empty: document.getElementById('whPackEmpty'),
        pagination: document.getElementById('whPackPagination'),
        prev: document.getElementById('whPackPrev'),
        next: document.getElementById('whPackNext'),
        pageMeta: document.getElementById('whPackPageMeta'),
        detailModal: document.getElementById('whPackDetailModal'),
        detailClose: document.getElementById('whPackDetailClose'),
        detailTitle: document.getElementById('whPackDetailTitle'),
        detailSubtitle: document.getElementById('whPackDetailSubtitle'),
        detailBody: document.getElementById('whPackDetailBody'),
        toast: document.getElementById('whPackToast'),
    };

    function toast(msg, type = 'success') {
        if (!els.toast) return;
        els.toast.textContent = msg;
        els.toast.className = `wh-pack-toast show${type === 'error' ? ' wh-pack-toast--error' : ''}`;
    ...
    function statusBadge(status) {
        const cls = status === 'packed' ? 'ok' : 'warn';
    ...
    function setStatsLoading(loading) {
        document.querySelectorAll('.wh-pack-stat__value').forEach((el) => el.classList.toggle('is-loading', loading));
    }

    function renderStats(summary) {
        const s = summary || {};
        if (els.statQueue) els.statQueue.textContent = String(s.queue ?? 0);
        if (els.statPacked) els.statPacked.textContent = String(s.packed ?? 0);
        if (els.statTotal) els.statTotal.textContent = String(s.total ?? 0);
        if (els.heroMeta) {
            els.heroMeta.textContent = t('wh_pack_hero_meta', s.queue ?? 0, s.packed ?? 0);
        }
    ...
    function renderBreakdown(items) {
        ...
        const activeStatus = els.status?.value || 'packing_active';
        els.breakdownPanel.hidden = false;
        els.statusChips.innerHTML = sorted.map((r) => {
            const isActive = activeStatus === r.status;
            return `<button type="button" class="wh-pack-status-chip${isActive ? ' is-active' : ''}" data-status="${esc(r.status)}">
                <span>${esc(statusLabel(r.status))}</span>
                <strong>${Number(r.count || 0)}</strong>
            </button>`;
        }).join('');
        els.statusChips.querySelectorAll('.wh-pack-status-chip').forEach((btn) => {
            btn.addEventListener('click', () => {
                if (els.status) els.status.value = btn.dataset.status || 'packing_active';
                state.page = 1;
                load();
            });
        });
    ...
    function renderTable(items) {
        ...
        tableWrap.innerHTML = `<table class="modern-table wh-table wh-pack-table">
<thead><tr>
...
<tbody>${items.map((d) => {
    const canMarkPacked = canPack && d.status === 'picking';
    return `<tr>
        ...
        <td class="wh-pack-row-actions">
            <button type="button" class="wh-btn wh-btn--ghost wh-btn--sm" data-pack-view="${d.id}">${esc(t('wms_view_details'))}</button>
            ${canMarkPacked ? `<button type="button" class="wh-btn wh-btn--sm wh-btn--primary" data-pack-mark="${d.id}">${esc(t('wms_mark_packed'))}</button>` : ''}
            ${d.status === 'packed' ? `<span class="wh-pack-ready-tag">${esc(t('wh_pack_ready_badge'))}</span>` : ''}
        </td>
    </tr>`;
}).join('')}</tbody></table>`;

        tableWrap.querySelectorAll('[data-pack-view]').forEach((btn) => {
            btn.addEventListener('click', () => openDetail(Number(btn.dataset.packView)));
        });
        tableWrap.querySelectorAll('[data-pack-mark]').forEach((btn) => {
            btn.addEventListener('click', () => markPacked(Number(btn.dataset.packMark)));
        });
    }
    ...
    async function exportData() {
        try {
            ...
            exportCsv(`packing-${new Date().toISOString().slice(0, 10)}.csv`, buildExportRows(items));
        } catch (e) {
            showError(e.message || t('load_error'));
        }
    }
    ...
    async function markPacked(id, fromDetail = false) {
        if (!window.confirm(t('wms_confirm_pack'))) return;
        const res = await AdminAPI.updateWmsDispatchStatus(id, 'packed');
        if (res.status !== 'success') {
            showError(res.message || t('error'));
            return;
        }
        hideError();
        toast(t('wh_pack_toast_packed'));
    ...
    async function openDetail(id) {
        ...
        try {
            ...
            const items = d.items || [];
            const canMark = canPack && d.status === 'picking';
            els.detailBody.innerHTML = `
                <dl class="wh-pack-detail-grid">
                ...
                ${d.notes ? `<p class="wh-pack-detail-notes"><strong>${esc(t('wms_receipt_notes'))}:</strong> ${esc(d.notes)}</p>` : ''}
                <div class="wh-pack-lines-wrap">
                    <table class="modern-table wh-table wh-pack-lines-table">
                        <thead><tr>
                            <th class="wh-pack-check-col"></th>
                            <th>${esc(t('wms_col_product'))}</th>
                            <th>${esc(t('wms_col_sku'))}</th>
                            <th>${esc(t('wms_col_qty'))}</th>
                        </tr></thead>
                        <tbody>${items.map((i) => `<tr>
                            <td class="wh-pack-check-col"><span class="material-icons-round wh-pack-check-icon" aria-hidden="true">check_box_outline_blank</span></td>
                ...
                ${canMark ? `<div class="wh-pack-detail-actions">
                    <button type="button" class="wh-btn wh-btn--primary" id="whPackMarkBtn" data-id="${d.id}">${esc(t('wms_mark_packed'))}</button>
                </div>` : ''}
                ${d.status === 'packed' ? `<p class="wh-pack-ready-note"><span class="material-icons-round">local_shipping</span> ${esc(t('wh_pack_ready_badge'))}</p>` : ''}`;
            document.getElementById('whPackMarkBtn')?.addEventListener('click', () => {
                markPacked(Number(d.id), true);
            });
        } catch (e) {
            els.detailBody.innerHTML = `<p class="wh-pack-empty-inline">${esc(e.message || t('load_error'))}</p>`;
        }
    }
